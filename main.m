...
if (automatic == 0) %diferentes overlaps (mismo frame)
    ...
    %Metrics
    manual_rmseVal = sqrt(immse(montage, ground_Truth)); %1.RMSE
    rmseVal = rmseFun(montage, ground_Truth);
    fprintf('RMSE = %.2f (The lower de better)\n',manual_rmseVal);
    fprintf('RMSE = %.2f (The lower de better)\n',rmseVal);
    
    manual_psnrVal = psnrFun(montage, ground_Truth);%2.PSNR
    psnrVal = psnrFun(montage, ground_Truth,rmseVal);
    fprintf('PSNR = %.2f (The higher de better)\n',manual_psnrVal);
    fprintf('PSNR = %.2f (The higher de better)\n',psnrVal);
    
    manual_ssimVal = ssim(montage,ground_Truth); %3.SSIM
    ssimVal = ssimFun(montage,ground_Truth);
